...
import type { FacialHairColorConstructor } from './Colors'
import { setAvailableTypeFor } from '../../../options/availableOptions'

setAvailableTypeFor &&
  setAvailableTypeFor('MoustacheMagnum', ['facialHairColor'])

export interface MoustacheMagnumProps {
  facialHairColor?: keyof typeof Colors
}

export const MoustacheMagnum: React.FC<MoustacheMagnumProps> = ({
  facialHairColor,
}) => {
  const mask1 = uniqueId('react-mask-')
  const path1 = uniqueId('react-path-')

  const Color: FacialHairColorConstructor = facialHairColor
    ? Colors[facialHairColor]
    : Colors.BrownDark

  return (
    <g
      id='FacialHair/Moustache-Magnum'
      transform='translate(49.000000, 72.000000)'
      data-testid='FacialHair/MoustacheMagnum'>
      <defs>
        <path
