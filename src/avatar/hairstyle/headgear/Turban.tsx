...
import { Colors as HeadgearColors, isHeadGearColor } from './Color'
import { HEADGEAR } from '../Category'
import { TopProps } from '../Top'

export const category = HEADGEAR

export const associatedTypes = ['headgearColor', 'facialHairType']

export const Turban: React.FC<TopProps> = ({ children, color, opacity }) => {
  ...
  const HeadgearColor: React.FC<{
    maskID: string
  }> = isHeadGearColor(color) ? HeadgearColors[color] : HeadgearColors.Blue03

  return (
    <g id='Style' strokeWidth='1' fillRule='evenodd' data-testid='Style/Turban'>
      <defs>
      ...
      <mask id={mask1} fill='white' opacity={opacity}>
        <use xlinkHref={'#' + path1} />
      </mask>
      <g id='Mask' />
      <g id='Style/Accesories/Turban' mask={`url(#${mask1})`}>
