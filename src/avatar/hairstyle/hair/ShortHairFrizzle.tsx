...
import { Colors as HairColors, isHairColor } from './HairColor'
import { SHORT_HAIR } from '../Category'
import { TopProps } from '../Top'

export const category = SHORT_HAIR

export const associatedTypes = ['hairColor', 'facialHairType']

export const ShortHairFrizzle: React.FC<TopProps> = ({
  children,
  color,
  opacity,
}) => {
  ...
  const HairColor = isHairColor(color)
    ? HairColors[color]
    : HairColors.BrownDark
  return (
    <g
      id='Style'
      strokeWidth='1'
      fillRule='evenodd'
      data-testid='Style/ShortHairFrizzle'>
      <defs>
      ...
      <mask id={mask2} fill='white' opacity={opacity}>
        <use xlinkHref={'#' + path2} />
      </mask>
      <g id='Mask' />
      <g id='Style/Short-Hair/Frizzle' mask={`url(#${mask2})`}>
...
export const Component = ShortHairFrizzle
